...
var (
	EtcdClusterLabelKey       = "etcd.yf.io/cluster"
...
func MutateHeadlessSvc(cluster *etcdv1alpha1.EtcdCluster, svc *corev1.Service) {
	...
	svc.Spec = corev1.ServiceSpec{
		...
		// map 值有哪些？就是Selector属性，映射过来即可，可以提前定义全局Key，自己规范的
...
func MutateStatefulSet(cluster *etcdv1alpha1.EtcdCluster, sts *appsv1.StatefulSet) {
	...
	sts.Spec = appsv1.StatefulSetSpec{
		// Replicas 读取 cluster的 size 大小
		Replicas:    cluster.Spec.Size,
		ServiceName: cluster.Name,
		Selector: &metav1.LabelSelector{
			MatchLabels: map[string]string{
				EtcdClusterLabelKey: cluster.Name, // 这样匹配到这个标签了
			},
		},
		Template: corev1.PodTemplateSpec{
			ObjectMeta: metav1.ObjectMeta{
				Labels: map[string]string{
					EtcdClusterLabelKey:       cluster.Name, // 标签关联
					EtcdClusterCommonLabelKey: "etcd",
				},
			},
			Spec: corev1.PodSpec{
				Containers: newContainers(cluster), // 写函数组装 containers
			},
		},
